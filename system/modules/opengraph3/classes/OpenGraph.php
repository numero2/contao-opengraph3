...
class OpenGraph3 extends \Frontend {
	...
	public static function setPageOGTags( $ref=NULL )
	{
		...
		// og:title
		if( $objRef->og_title || $objRootPage->og_title )
		{
		...
		// og:type
		if( $objRef->og_type || $objRootPage->og_type )
		{
		...
		// og:description
		if( $objRef->og_description || $objRootPage->og_description )
		{
		...
		// og:site_name
		if( $objRef->og_site_name || $objRootPage->og_site_name )
		{
		...
		// og:locality
		if( $objRef->og_locality || $objRootPage->og_locality )
		{
		...
		// og:image
		if( $objRef->og_image || $objRootPage->og_image )
		{
		...
		// twitter:site
		if( $objRef->twitter_site || $objRootPage->twitter_site )
		{
		...
		// twitter:creator
		if( $objRef->twitter_creator || $objRootPage->twitter_creator )
		{
		...
		// twitter:title
		if( $objRef->twitter_title || $objRootPage->twitter_title )
		{
		...
		// twitter:description
		if( $objRef->twitter_description || $objRootPage->twitter_description )
		{
		...
		// twitter:image
		if( $objRef->twitter_image || $objRootPage->twitter_image )
		{
